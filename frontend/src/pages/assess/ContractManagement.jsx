...
const SUBSECTIONS = [
  {
    id: '2.1', title: 'The Management Team',
    criteria: [
      { key: 'mgmt_structure', label: 'Structure, organisations and skills of the management team', pts: 10 },
      { key: 'mgmt_knowhow',   label: 'Contract specific know how of the management team',           pts: 10 },
    ],
  },
  {
    id: '2.2', title: 'The On-Site Contract Manager',
    criteria: [
      { key: 'cm_skills',    label: 'Skills and experience',     pts: 8 },
      { key: 'cm_knowhow',   label: 'Contract specific know how', pts: 8 },
      { key: 'cm_training',  label: 'Contract specific training', pts: 8 },
      { key: 'cm_avail',     label: 'Availability',               pts: 6 },
      { key: 'cm_response',  label: 'Response time',              pts: 6 },
    ],
  },
  {
    id: '2.3', title: 'Rostering',
    criteria: [
      { key: 'ros_method',     label: 'Rostering Methodology',                pts: 8 },
      { key: 'ros_backup',     label: 'Back-up capacity',                      pts: 7 },
      { key: 'ros_procedures', label: 'General and client-specific procedures', pts: 7 },
      { key: 'ros_reporting',  label: 'Reporting',                             pts: 6 },
      { key: 'ros_contact',    label: 'Client contact',                        pts: 6 },
    ],
  },
  {
    id: '2.4', title: 'Support Services',
    criteria: [
      { key: 'sup_inspections', label: 'Inspections',    pts: 5 },
      { key: 'sup_hq',          label: 'HQ Support',     pts: 4 },
      { key: 'sup_other',       label: 'Other criteria', pts: 1 },
    ],
  },
];
...
const STORAGE_KEY  = 'assess_contract_v2';
...
export default function ContractManagement() {
  ...
  // Load individual evaluator scores
  useEffect(() => {
    ...
    if (activeEvaluator && activeBidder && selectedTender) {
      ...
      try {
        await axios.post(`${API}/evaluators/${activeEvaluator}/scores`, {
          tender_id: selectedTender, bidder_id: activeBidder.id, section: SECTION, scores: scoreRows,
        });
  ...
  if (bidders.length === 0) return (
    <div className="page-shell">
      <div style={{ maxWidth: 700, width: '100%' }}>
        ...
        <h1 className="section-title">2. Contract Management</h1>
    ...
    <div className="page-shell gp-shell">
      <div className="gp-wrapper">
        <div className="gp-topbar">
          <div>
            <h1 className="gp-title">2&nbsp;&nbsp;Contract Management / Operations</h1>
        ...
        <div className="ev-bar glass-card">
          <div className="ev-bar-left">
            ...
            {evaluators.length === 0 && (
              <button className="ev-bar-link" onClick={() => navigate('/evaluators')}><Users size={13} /> Register evaluators</button>
            )}
          </div>
