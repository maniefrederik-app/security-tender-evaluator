...
const SUBSECTIONS = [
  {
    id: '4.1', title: 'Structure and Organisation',
    criteria: [
      { key: 'str_org',      label: 'Organisation chart — clear reporting lines',                pts: 5 },
      { key: 'str_services', label: 'Range of security services offered',                        pts: 5 },
      { key: 'str_hse',      label: 'Health, safety and environmental resources',                pts: 5 },
      { key: 'str_proc',     label: 'Company procedures and quality management',                 pts: 5 },
      { key: 'str_control',  label: '24-hour control room support and escalation',               pts: 5 },
      { key: 'str_member',   label: 'Membership of trade associations (SASA, SASSETA, etc.)',    pts: 5 },
    ],
  },
  {
    id: '4.2', title: 'Security Philosophy',
    criteria: [
      { key: 'sec_philosophy', label: 'Security philosophy and approach to service delivery', pts: 10 },
      { key: 'sec_track',      label: 'Track record and contract retention rate',             pts: 10 },
    ],
  },
  {
    id: '4.3', title: 'Quality',
    criteria: [
      { key: 'qlt_system', label: 'Quality management — ISO 9001 or equivalent standard', pts: 10 },
    ],
  },
  {
    id: '4.4', title: 'Human Resource Management',
    criteria: [
      { key: 'hr_philosophy', label: 'HR philosophy, policies and people practices',          pts: 5 },
      { key: 'hr_headcount',  label: 'Number of employees (total, guards, supervisors)',      pts: 5 },
      { key: 'hr_turnover',   label: 'Staff turnover rate (lower is better)',                 pts: 5 },
      { key: 'hr_absences',   label: 'Absenteeism management',                               pts: 5 },
    ],
  },
  {
    id: '4.5', title: 'References',
    criteria: [
      { key: 'ref_sector',   label: 'Sector-related references (minimum 2)',              pts: 5 },
      { key: 'ref_contract', label: 'Contract-related references (same environment)',     pts: 5 },
      { key: 'ref_relevant', label: 'Relevant experience on similar tendered contracts', pts: 5 },
    ],
  },
  {
    id: '4.6', title: 'Certification and Awards',
    criteria: [
      { key: 'cert_iso',    label: 'Relevant certifications (ISO, OHSAS, etc.)', pts: 5 },
      { key: 'cert_awards', label: 'Industry awards or recognition',             pts: 5 },
    ],
  },
];
...
const STORAGE_KEY  = 'assess_company_v2';
...
export default function TheCompany() {
  ...
  const setLevel  = (k, v) => { if (!activeBidder) return; setScores(prev => ({ ...prev, [activeBidder.id]: { ...prev[activeBidder.id], [k]: v } })); };
  const totalFor  = (bidderId) => { const sc = scores[bidderId] || {}; return ALL_CRITERIA.reduce((sum, c) => sum + weightedPts(c.key, sc[c.key]), 0); };
  ...
  const handleReset = () => { if (!activeBidder) return; setScores(prev => ({ ...prev, [activeBidder.id]: emptyScores() })); };
  ...
  if (bidders.length === 0) return (
    <div className="page-shell"><div style={{ maxWidth: 700, width: '100%' }}>
      ...
      <h1 className="section-title">4. The Company</h1>
    ...
    <div className="page-shell gp-shell">
      <div className="gp-wrapper">
        <div className="gp-topbar">
          <div>
            <h1 className="gp-title">4&nbsp;&nbsp;The Company</h1>
